...
function Tasks({list,changeTitle,addTasks}) {
   const changeTitletask =()=>{
	...
	const taskComplite = (id,completed,e) => {
   ...
   return (
        <div className='task'>
            ...
            <div  className="task__wrap">
                {list.tasks && list.tasks.map((item,i) => (
                    
                    <div key={item.id} className="checkbox">
                      
                        <input  onChange={(e)=>taskComplite(item.id,item.completed,e)} defaultChecked={item.completed}  id={`check_${item.id}`} type="checkbox"/>
